...
let hanoi =
  parse_file "mini/milestone2/benchmarks/hanoi_benchmark/hanoi_benchmark.mini"

let test_tc t s =
  (* Kind of a hack, get move() and stick it in tenv  *)
  ...
  | `Stm ->
      print_endline
        (show_statement (check_stmt (parse statement s) tenv ret_typ))
